...
def load_people():
    ...
    For each row:
        - Section 1: build pre_college_experience from slots 1-3
...
# Convenience wrapper to enforce correct call order. load_sports() depends
# on load_compatibility() having run first, so callers should use this.
def load_all():
    load_compatibility()
    load_sports()
    load_people()
